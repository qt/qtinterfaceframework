    ...
    , m_client(client)
{
    ...
    connect(client, &QMqttClient::messageReceived, this, [this](const QByteArray &message, const QMqttTopicName &topic) {
        ...
        if (topic.name().startsWith("/echo/")) {
            echo(msgid.toUtf8(), message);
        } else if (topic.name().startsWith("/id/")) {
            id(msgid.toUtf8());
        } else if (topic.name().startsWith("/getCombo/")) {
            getCombo(msgid.toUtf8());
        } else if (topic.name().startsWith("/voidSlot/")) {
            voidSlot(msgid.toUtf8());
        } else if (topic.name().startsWith("/voidSlot2/")) {
            voidSlot2(msgid.toUtf8(), message);
        } else if (topic.name().startsWith("/timer/")) {
            timer(msgid.toUtf8(), message);
        } else if (topic.name().startsWith("/flagMethod/")) {
            flagMethod(msgid.toUtf8(), message);
        } else if (topic.name().startsWith("/enumMethod/")) {
...
{
    m_intValue = intValue;
    m_client->publish(QMqttTopicName("/intValue"), QByteArray::number(intValue));
}

void EchoService::setVarValue(const QVariant &varValue)
{
    m_varValue = varValue;
    m_client->publish(QMqttTopicName("/varValue"), varValue.toByteArray());
}

void EchoService::setEchoEnabled(bool echoEnabled)
{
    m_echoEnabled = echoEnabled;
    m_client->publish(QMqttTopicName("/echoEnabled"), QByteArray(echoEnabled ? "1" : "0"));
}

void EchoService::setFloatValue1(qreal floatValue1)
{
    m_floatValue1 = floatValue1;
    m_client->publish(QMqttTopicName("/floatValue1"), QByteArray::number(floatValue1, 10, QLocale::FloatingPointShortest));
}

void EchoService::setFloatValue2(qreal floatValue2)
{
    m_floatValue2 = floatValue2;
    m_client->publish(QMqttTopicName("/floatValue2"), QByteArray::number(floatValue2, 10, QLocale::FloatingPointShortest));
}

void EchoService::setStringValue(const QString &stringValue)
{
    m_stringValue = stringValue;
    m_client->publish(QMqttTopicName("/stringValue"), stringValue.toUtf8());
}

void EchoService::setContact(const Contact &contact)
{
    m_contact = contact;
    m_client->publish(QMqttTopicName("/contact"), QJsonDocument::fromVariant(contact.toVariant()).toJson(QJsonDocument::Compact));
}

void EchoService::setCombo(const Combo &combo)
{
    m_combo = combo;
    m_client->publish(QMqttTopicName("/combo"), QJsonDocument::fromVariant(combo.toVariant()).toJson(QJsonDocument::Compact));
}
...
void EchoService::setIntList(const QList<int> &intList)
{
    ...
    m_client->publish(QMqttTopicName("/intList"), value);
}
...
void EchoService::setComboList(const QList<Combo> &comboList)
{
    ...
    QByteArray value = QJsonDocument::fromVariant(list).toJson(QJsonDocument::Compact);
    m_client->publish(QMqttTopicName("/comboList"), value);
}

void EchoService::setWeekDay(Echomodule::DaysOfTheWeek weekDay)
{
    m_weekDay = weekDay;
    m_client->publish(QMqttTopicName("/weekDay"), QByteArray::number(int(weekDay)));
}

void EchoService::setTestEnum(Echomodule::TestEnum testEnum)
{
    m_testEnum = testEnum;
    m_client->publish(QMqttTopicName("/testEnum"), QByteArray::number(int(testEnum)));
}

void EchoService::setUPPERCASEPROPERTY(qreal UPPERCASEPROPERTY)
{
    m_UPPERCASEPROPERTY = UPPERCASEPROPERTY;
    m_client->publish(QMqttTopicName("/UPPERCASEPROPERTY"), QByteArray::number(UPPERCASEPROPERTY));
}
...
void EchoService::setStringInAStructProperty(const OnlyAStringInAStruct &stringInAStructProperty)
{
    ...
    m_client->publish(QMqttTopicName("/stringInAStructProperty"),
...
void EchoService::setLastMessage(const QString &lastMessage)
{
    ...
    m_client->publish(QMqttTopicName("/lastMessage"), lastMessage.toLatin1());
}
...
void EchoService::echo(const QByteArray &id, const QString &msg)
{
    ...
    m_client->publish(QMqttTopicName("/echo_results/" + id), msgValue.toUtf8());
}

void EchoService::id(const QByteArray &id)
{
    m_client->publish(QMqttTopicName("/id_results/" + id), m_testId.toUtf8());
}

void EchoService::getCombo(const QByteArray &id)
{
    auto document = QJsonDocument::fromVariant(m_testCombo.toVariant());
    m_client->publish(QMqttTopicName("/getCombo_results/" + id), document.toJson(QJsonDocument::Compact));
}

void EchoService::voidSlot(const QByteArray &id)
{
    m_client->publish(QMqttTopicName("/voidSlot_results/" + id), "1");
}

void EchoService::voidSlot2(const QByteArray &id, const QString &msg)
{
    Q_UNUSED(msg);
    m_client->publish(QMqttTopicName("/voidSlot2_results/" + id), "1");
}
...
void EchoService::timer(const QByteArray &id, const QString &msg)
{
    ...
    QTimer::singleShot(interval, this, [this, id](){
        ...
        m_client->publish(QMqttTopicName("/timer_results/" + id), "1");
    });
}
...
void EchoService::flagMethod(const QByteArray &id, const QString &msg)
{
    ...
    m_client->publish(QMqttTopicName("/flagMethod_results/" + id), directionValue);
}
...
void EchoService::enumMethod(const QByteArray &id, const QString &msg)
{
    ...
    m_client->publish(QMqttTopicName("/enumMethod_results/" + id), testEnumValue);
}

void EchoService::somethingHappened()
{
    m_client->publish(QMqttTopicName("/somethingHappened"), "1");
}

void EchoService::foobar(const QString &string)
{
    m_client->publish(QMqttTopicName("/foobar"), string.toUtf8());
}
...
void EchoService::anotherChanged(const AnotherStruct &anotherStruct)
{
    ...
    m_client->publish(QMqttTopicName("/anotherChanged"), document.toJson(QJsonDocument::Compact));
}
